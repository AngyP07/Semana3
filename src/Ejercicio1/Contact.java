...
public class Contact implements Comparable<Contact> {
    ...
    public Contact(String name, String email, String phone) {
        this.name = name; this.email = email; this.phone = phone;  // ← hueco 1
    }
    ...
    public int compareTo(Contact other) {
        return this.name.compareTo(other.name);  // ← hueco 2
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(this.email, c.email);  // ← hueco 3
    }

    @Override
    public int hashCode() {
        return Objects.hash(email);  // ← hueco 4
    }
...
class ContactManager {
    ...
    public boolean addContact(Contact contact) {
        return contacts.add(contact);  // ← hueco 5
    }

    public Optional<Contact> findByEmail(String email) {
        return contacts.stream()
                .filter(c -> c.getEmail().equals(email))  // ← hueco 6
                .findFirst();
    }
    ...
    public List<Contact> findByNamePrefix(String prefix) {
        return contacts.stream()
                .filter(c -> c.getName().toLowerCase()
                        .startsWith(prefix.toLowerCase()))  // ← hueco 7
